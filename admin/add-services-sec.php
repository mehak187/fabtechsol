...
<head>
    ...
    <title>Add services section</title>
        ...
        <div class="container">
            <div class="add-news">
                <?php
                    ...
                    $sel = "SELECT * FROM service_sec" or die("conn failed");
                ...
                ?>
                    <form action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="POST"
                        ...
                        <div class="field">
                            ...
                            <input type="text" placeholder="Our Services" name="heading" id="heading" value="<?php echo $row['heading'] ?>" required>
                        </div>

                        <div class="field">
                            <label for="sub_heading">Sub heading</label>
                            <input type="text" placeholder="We provide the best IT Solution Services" value="<?php echo $row['sub_heading'] ?>" name="sub_heading" id="sub_heading" required>
                        </div>
        ...
        if(isset($_POST['submit'])) { 
            ...
            $des = mysqli_real_escape_string($conn,$_POST['des']);
                ...
                if(mysqli_num_rows($res_sql)>0) {
                    $up = "update service_sec set heading='$heading',sub_heading='$sub_heading',des='$des'";
                    $res_up = mysqli_query($conn,$up);        
                    ?><script>
                    swal("", "Content updated successfully","success");
                    </script> <?php 
                }
                else {
                    $insert_sql = "INSERT INTO service_sec(heading,sub_heading,des) Values('$heading','$sub_heading','$des')" or die("conn failed");
                    $res = mysqli_query($conn,$insert_sql);
                    ?><script>
                    swal("", "Content added successfully","success");
                    </script><?php
                }
        } 
    ?>