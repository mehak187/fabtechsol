    ...
    <div class="contact-details-sec contact-us-sec">
        <div class="container">
           <?php 
             ...
             $id2 = $_GET['id'];
             $sql = "select * FROM why_fab_points where id = $id2";
             $res1 = mysqli_query($conn,$sql);
             $row1 = mysqli_fetch_assoc($res1); 
             
            if(isset($_POST['submit'])) {
                $point_name = mysqli_real_escape_string($conn,$_POST['point_name']);
                $file = $_FILES['img'];
                $file_name = $file['name'];
                ...
                $up_sql = "update why_fab_points set point_name='$point_name',icon='$dest_file' WHERE id=$ser_id" or die("query failed");
                $res = mysqli_query($conn,$up_sql);
                ?>
                <script>window.location="https://fabtechsol.com/admin/manage-why-choose.php";</script>
                <?php
                $_SESSION['EDIT'] = "hey2";
            ...
            <div class="add-news">
                    <form action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="POST"
                        ...
                        <h4>Add Why Choose Points</h4>
                        <input type="hidden" placeholder="" name="ser_id" id="ser_id" value="<?php echo $row1['id'] ?>">
                        <div class="field">
                            <label for="point_name">Point Name</label>
                            <input type="text" placeholder="e.g Dedicated Team" name="point_name" id="point_name" required>
                        </div>
                        <div class="field">
                            <label for="img">Icon</label>
                            <input type="file" name="img" id="img" required>
                            <p class="note">Recommended size:43 X 43</p>
                        </div>
