...
<head>
    ...
    <title>Add quality policy</title>
        ...
        <div class="container">
            <div class="add-news">
                    <form action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="POST"
                        ...
                        <h4>Add quality policy</h4>
                        <div class="field">
                            <label for="des">Description</label>
                            <textarea placeholder="We invest in our human resource by providing them a conducive work environment and by providing them regular training, guidelines for skill development." name="des" id="des" required></textarea>
                        </div>
    ...
    <?php
        ...
        if(isset($_POST['submit'])) {
            $des = mysqli_real_escape_string($conn,$_POST['des']);
                $insert_sql = "INSERT INTO quality_policy(des) Values('$des')" or die("conn failed");
                $res = mysqli_query($conn,$insert_sql);
                ?><script>
                swal("", "Quality policy added successfully","success");
                </script>
            <?php 
            } ?>
    </body>
