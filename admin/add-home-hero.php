        ...
        <div class="container">
            <div class="add-news">
                    <form action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="POST"
                        ...
                        <h4>Add Home-Hero Section</h4>
                        <div class="field">
                            <label for="hero_title">Hero Title</label>
                            <input type="text" placeholder="Hero Title" name="hero_title" id="hero_title" required>
                        </div>

                        <div class="field">
                            <label for="hero_des">Hero Paragraph 1</label>
                            <textarea placeholder="Hero Paragraph 1" name="hero_des" id="hero_des" required></textarea>
                        </div>

                        <div class="field">
                            <label for="hero_des2">Hero Paragraph 2</label>
                            <textarea placeholder="Hero Paragraph 2" name="hero_des2" id="hero_des2" required></textarea>
                        </div>
...
<?php
        ...
        if(isset($_POST['submit'])) { 
            ...
            $hero_des = mysqli_real_escape_string($conn,$_POST['hero_des']);
            $hero_des2 = mysqli_real_escape_string($conn,$_POST['hero_des2']);
           
            $sel = "SELECT * FROM home_hero" or die("conn failed");
            $res_sql = mysqli_query($conn,$sel);
            $row = mysqli_fetch_assoc($res_sql);
            
                if(mysqli_num_rows($res_sql)>0) {
                    $up = "update home_hero set home_hero_title='$hero_title',home_hero_des='$hero_des',home_hero_des2='$hero_des2'";
                    $res_up = mysqli_query($conn,$up);        
                    ?><script>
                    swal("", "Hero section's content updated successfully","success");
                    </script> <?php 
                }
                else {
                    $insert_sql = "INSERT INTO home_hero(home_hero_title,home_hero_des,home_hero_des2) Values('$home_hero_title','$home_hero_des','$hero_des2')" or die("conn failed");
                    $res = mysqli_query($conn,$insert_sql);
                    ?><script>
                    swal("", "Hero section's content added successfully","success");
                    </script><?php
                }
        } 
    ?>