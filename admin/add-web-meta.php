...
<head>
    ...
    <title>Add web development meta</title>
        ...
        <div class="container">
            <div class="add-news">
                    <form action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="POST"
                        ...
                        <h4>Add Web Development Meta</h4>
...
// ----------add-about-----
        ...
        if(isset($_POST['submit'])) { 
            ...
            $sel = "SELECT * FROM web_meta" or die("conn failed");
            $res_sql = mysqli_query($conn,$sel);
            $row = mysqli_fetch_assoc($res_sql);
                if(mysqli_num_rows($res_sql)>0) {
                    $up = "update web_meta set meta_des='$meta_des'";
                    $res_up = mysqli_query($conn,$up);        
                    ?><script>
                    swal("", "Web development meta description updated successfully","success");
                    </script> <?php 
                }
                else {
                    $insert_sql = "INSERT INTO web_meta(meta_des) Values('$meta_des')" or die("conn failed");
                    $res = mysqli_query($conn,$insert_sql);
                    ?><script>
                    swal("", "Web development meta description added successfully","success");
                    </script><?php
                }
        } 
    ?>