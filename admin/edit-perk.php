    ...
    <div class="contact-details-sec contact-us-sec">
        <div class="container">
           <?php 
             ...
             $id2 = $_GET['id'];
             $sql = "select * FROM perk where id = $id2";
             $res1 = mysqli_query($conn,$sql);
             $row1 = mysqli_fetch_assoc($res1); 
             
            if(isset($_POST['submit'])) {
                $perk_name =mysqli_real_escape_string($conn,$_POST['perk_name']);

                $ser_id = $_POST['ser_id'];
                    
                $up_sql = "update perk set perk_name='$perk_name' WHERE id=$ser_id" or die("query failed");
                $res = mysqli_query($conn,$up_sql);
                ?>
                <script>window.location="https://fabtechsol.com/admin/manage-about-com-home.php";</script>
                <?php
                $_SESSION['EDIT'] = "hey2";
            ...
            <div class="add-news">
                    <form action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="POST"
                        ...
                        <div class="field">
                            <label for="perk_name">Perk Name</label>
                            <input type="text" placeholder="e.g Free requirement analysis session" name="perk_name" id="perk_name" required>
                        </div>
