    ...
    <div class="contact-details-sec contact-us-sec">
        <div class="container">
           <?php 
             ...
             $id2 = $_GET['id'];
             echo $id2;
             $sql = "SELECT * FROM client_tbl where id = $id2" or die("query failed");
             $res1 = mysqli_query($conn,$sql);
             $row1 = mysqli_fetch_assoc($res1); 
             
            if(isset($_POST['submit'])) {
                $client_name = $_POST['client_name'];
                $client_para =mysqli_real_escape_string($conn,$_POST['client_para']);
                $client_id = $_POST['client_id'];
                $client_img_alt = $_POST['client_img_alt'];
                
                $file = $_FILES['client_img'];
                $file_name = $file['name'];
                ...
                if(in_array($file_lower,$file_main)){
                    ...
                    $up_sql = "update client_tbl set client_name='$client_name', client_para='$client_para', client_img='$dest_file', client_img_alt='$client_img_alt' WHERE id=$client_id" or die("query failed");
                    $res = mysqli_query($conn,$up_sql);
                    ?>
                    <script>window.location="https://fabtechsol.com/admin/manage-review.php";</script>
                    <?php
                    $_SESSION['EDIT'] = "hey2";
                } 
                else{
            ...
            <div class="add-news">
                    ...
                    <form action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="POST"
                        ...
                        <h4>Edit Review</h4>
                        <input type="hidden" placeholder="" name="client_id" id="client_id" value="<?php echo $row1['id'] ?>">
                        <div class="field">
                            <label for="client_name">Client Name</label>
                            <input type="text" placeholder="" name="client_name" id="client_name" value="<?php echo $row1['client_name'] ?>" required>
                        </div>
                        <div class="field">
                            <label for="client_para">Client Review</label>
                            <textarea  placeholder="" name="client_para" id="client_para" required><?php echo $row1['client_para'] ?></textarea>
                        </div>
                        <div class="field">
                            <label for="client_alt">Client Image Alt</label>
                            <input type="text" placeholder="" name="client_img_alt" id="client_alt" value="<?php echo $row1['client_img_alt'] ?>" required>
                        </div>
                        <div class="field">
                            <label for="client_img">Client Image</label>
                            <input type="file" name="client_img" id="client_img" value="<?php echo $row1['client_img'] ?>" required>
                            <p class="note">Recommended size:108 X 108</p>
                        </div>
