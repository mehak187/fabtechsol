   ...
   ?>
    ...
    <?php
        ...
        if(isset($_POST['submit'])) {
            ...
            $sel = "SELECT * FROM banner" or die("conn failed");
            $res_sql = mysqli_query($conn,$sel);
            $row = mysqli_fetch_assoc($res_sql);
            
                if(mysqli_num_rows($res_sql)>0) {
                    $up = "update banner set heading='$heading',des='$des',btn_name='$btn_name',btn_link='$btn_link'";
                    $res_up = mysqli_query($conn,$up);        
                    ?><script>
                    swal("", "Banner content updated successfully","success");
                    </script> <?php 
                } 
                else{
                    $insert_sql = "INSERT INTO banner(heading,des,btn_name,btn_link) Values('$heading','$des','$btn_name','$btn_link')" or die("conn failed");
                    $res = mysqli_query($conn,$insert_sql);
                    ?><script>
                    swal("", "Banner content added successfully","success");
                    </script><?php
                    } 
            }
            ?>
