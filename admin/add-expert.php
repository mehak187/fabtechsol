...
<head>
    ...
    <title>Add expert content</title>
   ...
   ?>
        ...
        <div class="container">
            ...
            <div class="add-news">
                    <form action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="POST"
                        ...
                        <h4>Add expert content</h4>
                        <div class="field">
                            <label for="heading">Heading</label>
                            <input type="text" placeholder="Heading" name="heading" id="heading" required>
                        </div>
                        <div class="field">
                            <label for="des">Description</label>
                            <textarea  placeholder="Description" name="des" id="des" required></textarea>
                        </div>
                        <div class="field">
                            <label for="btn_name">Button name</label>
                            <input type="text" placeholder="Chat with our team" name="btn_name" id="btn_name" required>
                        </div>
                        <div class="field">
                            <label for="btn_link">Button Link (Page name or Link)</label>
                            <input type="text" placeholder="contact-us" name="btn_link" id="btn_link" required>
                        </div>
    ...
    <?php
        ...
        if(isset($_POST['submit'])) {
            ...
            $sel = "SELECT * FROM expert" or die("conn failed");
            $res_sql = mysqli_query($conn,$sel);
            $row = mysqli_fetch_assoc($res_sql);
            
                if(mysqli_num_rows($res_sql)>0) {
                    $up = "update expert set heading='$heading',des='$des',btn_name='$btn_name',btn_link='$btn_link'";
                    $res_up = mysqli_query($conn,$up);        
                    ?><script>
                    swal("", "Expert content updated successfully","success");
                    </script> <?php 
                } 
                else{
                    $insert_sql = "INSERT INTO expert(heading,des,btn_name,btn_link) Values('$heading','$des','$btn_name','$btn_link')" or die("conn failed");
                    $res = mysqli_query($conn,$insert_sql);
                    ?><script>
                    swal("", "Expert content added successfully","success");
                    </script><?php
                    } 
            }
            ?>
