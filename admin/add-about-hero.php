...
<?php
        ...
        if(isset($_POST['submit'])) { 
            ...
            $sel = "SELECT * FROM about_hero" or die("conn failed");
            $res_sql = mysqli_query($conn,$sel);
            $row = mysqli_fetch_assoc($res_sql);
            
                if(mysqli_num_rows($res_sql)>0) {
                    $up = "update about_hero set about_hero_title='$hero_title',about_hero_des='$hero_des'";
                    ...
                    $insert_sql = "INSERT INTO about_hero(about_hero_title,about_hero_des) Values('$hero_title','$hero_des')" or die("conn failed");
