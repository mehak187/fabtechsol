...
<head>
    ...
    <title>Add about ui</title>
    <link rel="icon" type="image/x-icon" href="../img/logo.png">
    <meta name="description"
        content="Fabulous Technology Solutions one of the Top IT Company provides services in Web uilication, Software development, Mobile app Development, Web Development, UI/UX design and Electrical Hardware Development .">
    <meta name="keyword"
   ...
   ?>
        ...
        <div class="container">
            ...
            <div class="add-news">
                    <form action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="POST"
                        ...
                        <h4>Add About UI/UX Service</h4>
                        <div class="field">
                            <label for="heading">Heading</label>
                            <input type="text" placeholder="UI & UX Design " name="heading" id="heading" required>
                        </div>
                        <div class="field">
                            <label for="sub_heading">Sub Heading</label>
                            <input type="text" placeholder="What is UI & UX Design? How this Process Completes." name="sub_heading" id="sub_heading" required>
                        </div>
                        <div class="field">
                            <label for="">Description</label>
                            <textarea  placeholder="UI/UX layout is a defining element for any virtual product nowadays. we are a user enjoy and UI design agency centered on enhancing conversion and increasing purchaser engagement. Our UI/UX design capabilities do not prevent at mobile apps, internet packages, or multi-platform virtual reports. As a UX layout agency, we create products and offerings that offer first rate usability at the same time as completely embracing your brand's personality " name="des" id="des" required></textarea>
                        </div>
                        <div class="field">
                            <label for="img_1">Image 1</label>
                            <input type="file" name="img_1" id="img_1" required>
                            <p class="note">Recommended size:468X401</p>
                        </div>
                        <div class="field">
                            <label for="img_2">Image 2</label>
                            <input type="file" name="img_2" id="img_2" required>
                            <p class="note">Recommended size:220X220</p>
                        </div>
    ...
    <?php
        ...
        if(isset($_POST['submit'])) {
            ...
            $sel = "SELECT * FROM about_ui" or die("conn failed");
            ...
            if((in_array($file_lower,$file_main)) && (in_array($file_lower2,$file_main2))) {
                ...
                if(mysqli_num_rows($res_sql)>0) {
                    $up = "update about_ui set heading='$heading',sub_heading='$sub_heading',des='$des',img_1='$dest_file',img_2='$dest_file2'";
                    $res_up = mysqli_query($conn,$up);        
                    ?><script>
                    swal("", "About ui content updated successfully","success");
                    </script> <?php 
                } 
                else{
                    $insert_sql = "INSERT INTO about_ui(heading,sub_heading,des,img_1,img_2) Values('$heading','$sub_heading','$des','$dest_file','$dest_file2')" or die("conn failed");
                    $res = mysqli_query($conn,$insert_sql);
                    ?><script>
                    swal("", "About ui content added successfully","success");
                    </script><?php
                    } 
            }
            else{
