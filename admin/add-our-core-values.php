...
<head>
    ...
    <title>Add Core Values Section</title>
        ...
        <div class="container">
            <div class="add-news">
                    <form action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="POST"
                        ...
                        <h4>Add Core Values Content</h4>
                        <div class="field">
                            <label for="heading">Heading</label>
                            <input type="text" placeholder="Our core values" name="heading" id="heading" required>
                        </div>
                        <div class="field">
                            <label for="des">Description</label>
                            <textarea placeholder="Our values are at the heart of what we do and how we do it." name="des" id="des" required></textarea>
                        </div>
...
// ----------add-about-----
        ...
        if(isset($_POST['submit'])) { 
            ...
            $sel = "SELECT * FROM our_core_values" or die("conn failed");
            $res_sql = mysqli_query($conn,$sel);
            $row = mysqli_fetch_assoc($res_sql);
                if(mysqli_num_rows($res_sql)>0) {
                    $up = "update our_core_values set heading='$heading', des='$des'";
                    $res_up = mysqli_query($conn,$up);        
                    ?><script>
                    swal("", "Our Core Values section updated successfully","success");
                    </script> <?php 
                }
                else {
                    $insert_sql = "INSERT INTO our_core_values(heading,des) Values('$heading','$des')" or die("conn failed");
                    $res = mysqli_query($conn,$insert_sql);
                    ?><script>
                    swal("", "Our Core Values section added successfully","success");
                    </script><?php
                }
        } 
    ?>