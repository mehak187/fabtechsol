...
<head>
    ...
    <title>Add software development meta</title>
        ...
        <div class="container">
            <div class="add-news">
                    <form action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="POST"
                        ...
                        <h4>Add Software Development Meta</h4>
                        
                        <div class="field">
                            <label for="meta_des">Meta Description (only description without tag)</label>
                            <textarea placeholder="FABTECHSOL Is A Global Web Design And Development Company That Provides Intelligent, Cost-Effective Web And IT Solutions. Thanks To Our Client-First Ideology, We've Gained Worldwide Recognition For The Quality Of Our Products, Services, And Solutions" name="meta_des" id="meta_des" required></textarea>
                        </div>
...
// ----------add-about-----
        ...
        if(isset($_POST['submit'])) { 
            $meta_des = mysqli_real_escape_string($conn,$_POST['meta_des']);
           
            $sel = "SELECT * FROM software_meta" or die("conn failed");
            $res_sql = mysqli_query($conn,$sel);
            $row = mysqli_fetch_assoc($res_sql);
                if(mysqli_num_rows($res_sql)>0) {
                    $up = "update software_meta set meta_des='$meta_des'";
                    $res_up = mysqli_query($conn,$up);        
                    ?><script>
                    swal("", "Software development meta description updated successfully","success");
                    </script> <?php 
                }
                else {
                    $insert_sql = "INSERT INTO software_meta(meta_des) Values('$meta_des')" or die("conn failed");
                    $res = mysqli_query($conn,$insert_sql);
                    ?><script>
                    swal("", "Software development meta description added successfully","success");
                    </script><?php
                }
        } 
    ?>