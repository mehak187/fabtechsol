        ...
        <div class="container">
            <div class="add-news">
                    <form action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="POST"
                        ...
                        <h4>Add Our Vision</h4>
                        <div class="field">
                            <label for="heading">Heading</label>
                            <input type="text" placeholder="Our vision" name="heading" id="heading" required>
                        </div>
                        <div class="field">
                            <label for="des">Description</label>
                            <textarea placeholder="To be a globally respected corporation that provides best-of-breed business solutions, leveraging technology, delivered by best-in-class people." name="des" id="des" required></textarea>
                        </div>
...
// ----------add-about-----
        ...
        if(isset($_POST['submit'])) { 
            ...
            $sel = "SELECT * FROM our_vision" or die("conn failed");
            $res_sql = mysqli_query($conn,$sel);
            $row = mysqli_fetch_assoc($res_sql);
                if(mysqli_num_rows($res_sql)>0) {
                    $up = "update our_vision set heading='$heading', des='$des'";
                    $res_up = mysqli_query($conn,$up);        
                    ?><script>
                    swal("", "Our vision section updated successfully","success");
                    </script> <?php 
                }
                else {
                    $insert_sql = "INSERT INTO our_vision(heading,des) Values('$heading','$des')" or die("conn failed");
                    $res = mysqli_query($conn,$insert_sql);
                    ?><script>
                    swal("", "Our vision section added successfully","success");
                    </script><?php
                }
        } 
    ?>