...
include 'connection.php';
    ...
    $del = "DELETE from blog_main WHERE id=$id";

    //select img from event tbl
    $sql1 = "SELECT img1,img2,img3 FROM blog_main WHERE id=$id";
    $result1 = mysqli_query($conn,$sql1);
    $row1= mysqli_fetch_assoc($result1);
    $image_paths = array($row1['img1'], $row1['img2'], $row1['img3']);

// ----------select blog ------
 $sql2 = "SELECT * FROM blog_main WHERE id=$id";
    $result2= mysqli_query($conn,$sql2);
    $row2= mysqli_fetch_assoc($result2);
    
    $filename = "../blogs/" . $row2['meta_con'] . ".php";
