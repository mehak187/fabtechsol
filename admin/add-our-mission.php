        ...
        <div class="container">
            <div class="add-news">
                    <form action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="POST"
                        ...
                        <h4>Add Our Mission</h4>
                        <div class="field">
                            <label for="heading">Heading</label>
                            <input type="text" placeholder="Our mission" name="heading" id="heading" required>
                        </div>
                        <div class="field">
                            <label for="des">Description</label>
                            <textarea placeholder="Our mission is to continue being the Artisan of technology, to help businesses achieve greater success through cutting-edge custom software development, global business intelligence, implementation, and consulting services." name="des" id="des" required></textarea>
                        </div>
...
// ----------add-about-----
        ...
        if(isset($_POST['submit'])) { 
            ...
            $sel = "SELECT * FROM our_mission" or die("conn failed");
            $res_sql = mysqli_query($conn,$sel);
            $row = mysqli_fetch_assoc($res_sql);
                if(mysqli_num_rows($res_sql)>0) {
                    $up = "update our_mission set heading='$heading', des='$des'";
                    $res_up = mysqli_query($conn,$up);        
                    ?><script>
                    swal("", "Our mission section updated successfully","success");
                    </script> <?php 
                }
                else {
                    $insert_sql = "INSERT INTO our_mission(heading,des) Values('$heading','$des')" or die("conn failed");
                    $res = mysqli_query($conn,$insert_sql);
                    ?><script>
                    swal("", "Our mission section added successfully","success");
                    </script><?php
                }
        } 
    ?>