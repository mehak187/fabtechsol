    ...
    <div class="contact-details-sec contact-us-sec">
        <div class="container">
           <?php 
             ...
             $id2 = $_GET['id'];
             $sql = "SELECT * FROM core_value_box where id = $id2";
            ...
            if(isset($_POST['submit'])) {
                ...
                $up_sql = "update core_value_box set heading='$heading', des='$des' WHERE id=$tech_id" or die("query failed");
                $res = mysqli_query($conn,$up_sql);
                ?>
                <script>window.location="https://fabtechsol.com/admin/manage-blog.php";</script>
                <?php
                $_SESSION['EDIT'] = "hey2";
            } ?>
            <div class="add-news">
                    <form action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="POST"
                        ...
                        <h4>Edit core value</h4>
                        <div>
                            <input type="hidden" placeholder="" name="tech_id" id="tech_id" value="<?php echo $row1['id'] ?>" required>
                        </div>
                        <div class="field">
                            <label for="heading">Heading</label>
                            <input type="text" placeholder="<?php echo $row1['heading'] ?>" name="heading" id="heading" required>
                        </div>
                        <div class="field">
                            <label for="des">Description</label>
                            <textarea placeholder="<?php echo $row1['des'] ?>" name="des" id="des" required></textarea>
                        </div>
