    ...
    <div class="contact-details-sec contact-us-sec">
        <div class="container">
           <?php 
             ...
             $id2 = $_GET['id'];
             $sql = "select * FROM courses_main where course_id = $id2";
             $res1 = mysqli_query($conn,$sql);
             $row1 = mysqli_fetch_assoc($res1); 
             
            if(isset($_POST['submit'])) {
                $course_title = $_POST['course_title'];
                $course_des =mysqli_real_escape_string($conn,$_POST['course_des']);

                $ser_id = $_POST['ser_id'];

                $file2 = $_FILES['course_image'];
                ...
                if(empty($file2['name'])){
                    $up_sql = "update courses_main set course_title='$course_title', course_des='$course_des' WHERE course_id=$ser_id" or die("query failed");
                    $res = mysqli_query($conn,$up_sql);
                    ?>
                    <script>window.location="https://fabtechsol.com/admin/manage-courses.php";</script>
                    <?php
                    $_SESSION['EDIT'] = "hey2";
                }
                else {
                    if(in_array($file_lower2,$file_main2)){
                        ...
                        $up_sql = "update courses_main set course_title='$course_title', course_des='$course_des', course_image='$dest_file2' WHERE course_id=$ser_id" or die("query failed");
                        $res = mysqli_query($conn,$up_sql);
                        ?>
                        <script>window.location="https://fabtechsol.com/admin/manage-courses.php";</script>
                        <?php
            ...
            <div class="add-news">
                    <form action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="POST"
                        ...
                        <h4>Edit Course</h4>
                        <input type="hidden" placeholder="" name="ser_id" id="ser_id" value="<?php echo $row1['course_id'] ?>">
                       <div class="field">
                            <label for="course_title">course Title</label>
                            <input type="text" placeholder="" value="<?php echo $row1['course_title'] ?>" name="course_title" id="course_title">
                        </div>
                        <div class="field">
                            <label for="course_des">course Description</label>
                            <textarea  placeholder="" name="course_des" id="course_des" required><?php echo $row1['course_des'] ?></textarea>
                        </div>
                        <div class="field">
                            <label for="course_image">course Image</label>
                            <input type="file" name="course_image" value="<?php echo $row1['course_image'] ?>" id="course_image">
                        </div>
