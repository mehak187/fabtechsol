   ...
   ?>
    ...
    <div class="contact-details-sec contact-us-sec">
        ...
        include 'connection.php';
            $sel = "SELECT * FROM why_fab" or die("conn failed");
            ...
            <div class="add-news">
                    <form action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="POST"
                        ...
                        <h4>Add Why Choose Content</h4>
                        <div class="field">
                            <label for="heading">Heading</label>
                            <input type="text" placeholder="Heading" name="heading" id="heading" value="<?php echo $row['heading'] ?>" required>
                        </div>
                        <div class="field">
                            <label for="sub_heading">Sub Heading</label>
                            <input type="text" placeholder="Sub Heading" name="sub_heading" id="sub_heading" value="<?php echo $row['sub_heading'] ?>" required>
                        </div>
                        <div class="field">
        ...
        if(isset($_POST['submit'])) {
            ...
            $des=mysqli_real_escape_string($conn,$_POST['des']);
                ...
                if(mysqli_num_rows($res_sql)>0) {
                    $up = "update why_fab set heading='$heading',sub_heading='$sub_heading',des='$des'";
                    $res_up = mysqli_query($conn,$up);        
                    ?><script>
                    swal("", "Why Choose content updated successfully","success");
                    </script> <?php 
                } 
                else{
                    $insert_sql = "INSERT INTO why_fab(heading,sub_heading,des) Values('$heading','$sub_heading','$des')" or die("conn failed");
                    $res = mysqli_query($conn,$insert_sql);
                    ?><script>
                    swal("", "Why Choose content added successfully","success");
                    </script><?php
                    } 
            }
            ?>
