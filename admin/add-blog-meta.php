...
// ----------add-about-----
        ...
        if(isset($_POST['submit'])) { 
            ...
            $sel = "SELECT * FROM blog_meta" or die("conn failed");
            $res_sql = mysqli_query($conn,$sel);
            $row = mysqli_fetch_assoc($res_sql);
                if(mysqli_num_rows($res_sql)>0) {
                    $up = "update blog_meta set meta_des='$meta_des'";
                    $res_up = mysqli_query($conn,$up);        
                    ?><script>
                    swal("", "Blog meta description updated successfully","success");
                    </script> <?php 
                }
                else {
                    $insert_sql = "INSERT INTO blog_meta(meta_des) Values('$meta_des')" or die("conn failed");
                    $res = mysqli_query($conn,$insert_sql);
                    ?><script>
                    swal("", "Blog meta description added successfully","success");
                    </script><?php
                }
        } 
    ?>