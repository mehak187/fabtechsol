...
// ----------add-about-----
        ...
        if(isset($_POST['submit'])) { 
            ...
            $sel = "SELECT * FROM about_meta" or die("conn failed");
            $res_sql = mysqli_query($conn,$sel);
            $row = mysqli_fetch_assoc($res_sql);
                if(mysqli_num_rows($res_sql)>0) {
                    $up = "update about_meta set meta_des='$meta_des'";
                    $res_up = mysqli_query($conn,$up);        
                    ?><script>
                    swal("", "About meta description updated successfully","success");
                    </script> <?php 
                }
                else {
                    $insert_sql = "INSERT INTO about_meta(meta_des) Values('$meta_des')" or die("conn failed");
                    $res = mysqli_query($conn,$insert_sql);
                    ?><script>
                    swal("", "About meta description added successfully","success");
                    </script><?php
                }
        } 
    ?>