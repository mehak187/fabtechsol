...
// ----------add-about-----
        ...
        if(isset($_POST['submit'])) { 
            ...
            $sel = "SELECT * FROM who_we_are" or die("conn failed");
            $res_sql = mysqli_query($conn,$sel);
            $row = mysqli_fetch_assoc($res_sql);
                if(mysqli_num_rows($res_sql)>0) {
                    $up = "update who_we_are set heading='$heading', des='$des'";
                    $res_up = mysqli_query($conn,$up);        
                    ?><script>
                    swal("", "Who we are section updated successfully","success");
                    </script> <?php 
                }
                else {
                    $insert_sql = "INSERT INTO who_we_are(heading,des) Values('$heading','$des')" or die("conn failed");
                    $res = mysqli_query($conn,$insert_sql);
                    ?><script>
                    swal("", "Who we are section added successfully","success");
                    </script><?php
                }
        } 
    ?>