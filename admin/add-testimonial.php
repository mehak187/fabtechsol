...
<body>
        ...
        <div class="container">
            <div class="add-news">
                    <form action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="POST"
                        ...
                        <h4>Add Testimonial Content</h4>
                        <div class="field">
                            <label for="heading">Heading</label>
                            <input type="text" placeholder="Testimonial" name="heading" id="heading" required>
                        </div>
                        <div class="field">
                            <label for="sub_heading">Sub Heading</label>
                            <input type="text" placeholder="What our customers are talking about" name="sub_heading" id="sub_heading" required>
                        </div>
                        <div class="field">
                            <label for="des">Description</label>
                            <textarea  placeholder="With hundreds of projects completed, fabtechsol developed an excellent sense of development structure and can ensure you to avoid early mistakes in going from vision to reality. To help you succeed, fabtechsol allocate a dedicated team of the best talent that effectively handles your workload and makes sure that we ship clean, well-documented code, on schedule." name="des" id="des" required></textarea>
                        </div>
    ...
    <?php
        ...
        if(isset($_POST['submit'])) {
            ...
            $sel = "SELECT * FROM testimonial" or die("conn failed");
            $res_sql = mysqli_query($conn,$sel);
            $row = mysqli_fetch_assoc($res_sql);
            
                if(mysqli_num_rows($res_sql)>0) {
                    $up = "update testimonial set heading='$heading',sub_heading='$sub_heading',des='$des'";
                    $res_up = mysqli_query($conn,$up);        
                    ?><script>
                    swal("", "Testimonial content updated successfully","success");
                    </script> <?php 
                } 
                else{
                    $insert_sql = "INSERT INTO testimonial(heading,sub_heading,des) Values('$heading','$sub_heading','$des')" or die("conn failed");
                    $res = mysqli_query($conn,$insert_sql);
                    ?><script>
                    swal("", "Testimonial content added successfully","success");
                    </script><?php
                    } 
            }
            ?>
